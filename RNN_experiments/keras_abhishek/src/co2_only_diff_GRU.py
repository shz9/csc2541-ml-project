...
def load_data_run_model(epochs_for_training=10,
						neuron_cells=12):
	...
	# GRU_model = fit_GRU(train_scaled, 1, 1000, 4)
	GRU_model = fit_GRU(train, 1, epochs_for_training, neuron_cells)
	...
	for i in range(len(test)):
		...
		if prev is None:
			prev = test[i, 0:-1]
		yhat = forecast_GRU(GRU_model, 1, prev)
		prev = yhat
...
def plot_fig(pred_arr, true):
	...
	plot_name='runs_'+str(len(final_list))+'_e_n_'+str(final_list)+'_co2_dataset_gru_test_on_137.pdf'
...
epochs_for_training = [100, 200, 500]
neuron_cells = [50, 100, 200, 300, 400]
